...
The live authenticated questionnaire journey slice drives two clean deterministic identities through every module-1 summative question: `questionnaire-pass@example.test` submits answers `2,1` and must finish at 100%/passed; `questionnaire-fail@example.test` submits `1,2` and must finish at 0%/failed. The failing identity then drives module 4 with answers `2,1,2`, producing a deterministic 0%/failed result. The module-4 check renders the feedback boundary in both applications and verifies that the failed result does not expose certificate eligibility; the fixture contract separately asserts the `feedback-q1` -> `thankyou` -> `certificate` order. The .NET training feedback form remains a separate parity gap, so the live check intentionally stops at rendering and does not claim feedback submission parity. It compares each submission redirect and final assessment-results response, while `parity/reconcile.ps1` proves the persisted assessments, summative responses, and Rails-owned assessment-start, answer, and completion events match across both databases. The only accepted route normalization is Rails' intermediate `/content-pages/summative-q2` redirect before the questionnaire route. The module-4 fixture addition proves focused synthetic navigation and grading only; it is intentionally not a production Contentful compatibility claim and remains incomplete against the full Rails/.NET ContentIntegrity contract. Real Contentful inventory and full release-valid module content remain required before cutover.

`./parity/reconcile.ps1` compares canonical row counts, ID bounds, completions, and result values across both databases. It also compares the questionnaire fixture user's passed assessment, numeric response attached to that assessment, and Rails-shaped assessment-start/answer event properties, plus the pass/fail questionnaire journey assessment, responses, completion events, certificate fixture progress keys/completion flags, unchanged assessment results, and event names. For the learning-log scenario it compares the deterministic note's user identity, title, training module, and page name without comparing encrypted body ciphertext; it separately compares the Rails-shaped `user_note_created` and `user_note_updated` event semantics and stable properties. The aggregate event count deliberately excludes the single Rails-only `feedback_start` event emitted when the synthetic feedback questionnaire is rendered; .NET's feedback form and telemetry remain an explicit parity gap. The report is written to `TestResults/database-reconciliation.json` and the script runs automatically by `./parity.ps1 test`.
