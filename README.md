...
The full-registration slice uses the fresh `full-registration@example.test` identity and drives both applications from terms acceptance to completion. It follows the England branch through `setting_la_general_role`, `local_authority_a`, the conditional custom-role page, `2-5` experience, training-email opt-out, research opt-in, check-your-answers, and final confirmation. A deliberately blank training-email submission must render 422 before a valid preference is saved. Rails `/edit` route suffixes are normalized to .NET's canonical registration paths. Reconciliation compares every persisted profile/preference field, exactly one successful `user_registration` event, and the absence of learning progress, assessments, or responses. Rails v1.5.0's caller-specific NULL-country behavior remains unchanged and is not generalized by this journey. A live exploratory run also found that blank-name validation returns 200 in .NET while Rails returns 422; that application defect is intentionally reported rather than fixed in this parity-only slice.

The authenticated account-closure slice uses the resettable `account-closure@example.test` identity populated with profile and email preferences, a learning-log note, feedback response, completed progress, passed assessment, historical event, Notify callback, and mail event. Both applications must reject a missing reason with 422, normalize a blank custom “Another reason” to `No reason provided`, persist a supplied custom reason, render confirmation/cancel semantics, emit the pinned learner and internal closure notifications, close the account, invalidate the local session, and render the public closed-account page. Rails development `NotifyDelivery` logs its two messages locally and does not call the HTTP recording fake; .NET must make exactly two fake calls using the pinned template IDs, original learner address, and configured internal mailbox. Reconciliation locates the row by its redacted GOV.UK One Login identifier and requires redacted identity, retained closure reason, cleared callback, deleted notes and mail events, nulled feedback free text, and otherwise unchanged response, progress, assessment, and event state. Generated redacted email/user IDs, closure timestamps, passwords, notification IDs, cookies, and framework markup are deliberately not compared.
